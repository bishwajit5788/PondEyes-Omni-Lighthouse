...
void setup() {
    ...
    delay(1000);
    Serial.println(F("=================================================="));
    Serial.println(F(" PondEyes Omni-Lighthouse: TEENSY NODE B STARTING"));
    Serial.println(F(" Node ID: 0x0B (Sensors: S4@180°, S5@240°, S6@300°)"));
    Serial.println(F(" UART Baudrate: 256,000 bps | 600 MHz Cortex-M7"));
    Serial.println(F("=================================================="));

    // Precompute trigonometric matrices for 180°, 240°, and 300°
    ...
    Serial.println(F("[Node B] Initialization complete. Entering high-speed DSP loop."));
}
...
void init_sensor_geometry() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        Serial.printf("[Node B] Sensor S%u: Angle=%.1f deg | Sin=%.4f, Cos=%.4f | MountOffset=(%.1f, %.1f) mm\n",
...
void loop() {
    ...
    if (now_ms - g_last_debug_print_ms >= 1000) {
        ...
        Serial.printf("[Node B Status] Uptime: %lu ms | Seq: %u | ValidTargets: %u | S4_pkts: %lu, S5_pkts: %lu, S6_pkts: %lu\n",
