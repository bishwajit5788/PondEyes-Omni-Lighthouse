...
void setup() {
    ...
    delay(1000);
    Serial.println(F("=================================================="));
    Serial.println(F(" PondEyes Omni-Lighthouse: TEENSY NODE A"));
    Serial.println(F(" Node ID: 0x0A | S1@0°, S2@60°, S3@120°"));
    ...
    Serial.println(F("[Node A] Initialization successful. System active."));
}
...
void init_sensor_geometry() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        Serial.printf("[Node A] Sensor S%u: Angle=%.1f° | MountOffset=(%.1f, %.1f) mm\n",
...
void loop() {
    ...
    if (now_ms - g_last_debug_print_ms >= 1000) {
        ...
        Serial.printf("[Node A] Uptime: %lu ms | Seq: %u | ActiveTargets: %u | S1: %lu pkts, S2: %lu pkts, S3: %lu pkts\n",
